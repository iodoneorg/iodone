...
contract UFragmentsPolicy is Ownable {
    ...
    // (eg) An oracle value of 1.5e18 it would mean 1 Iodone is trading for $1.50.
    ...
    /**
     * @notice A multi-chain IODN interface method. The Iodone monetary policy contract
     *         on the base-chain and XC-IodoneController contracts on the satellite-chains
     *         implement this method. It atomically returns two values:
     *         what the current contract believes to be,
     *         the globalIodoneEpoch and globalIODNSupply.
     * @return globalIodoneEpoch The current epoch number.
     * @return globalIODNSupply The total supply at the current epoch.
     */
    ...
    function computeSupplyDelta(uint256 rate, uint256 targetRate) internal view returns (int256) {
        ...
        return uFrags.totalSupply().toInt256Safe().mul(rate.toInt256Safe().sub(targetRateSigned)).div(targetRateSigned);
    }
